// forked from https://github.com/ggwhite/go-masker/blob/master/masker.go
package stringmasker
...
func (m *Masker) Email(i string) string {
	...
	tmp := strings.Split(i, "@")
	if len(tmp) == 1 {
		return m.overlay(i, strLoop(instance.mask, len("****")), 2, 7)
	}

	addr := tmp[0]
	domain := tmp[1]

	addr = m.overlay(addr, strLoop(instance.mask, len("****")), 2, 7)
